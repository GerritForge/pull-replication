...
import static com.gerritforge.gerrit.plugins.replication.pull.api.HttpServletOps.setResponse;
...
public class ProjectInitializationAction extends HttpServlet {
  ...
  protected void doPut(
    ...
    try {
      ...
      if (checkContentType(contentType, MediaType.JSON_UTF_8)) {
        ...
        initProjectStatus = initProject(gitRepositoryName, RefNames.HEAD, false);
      } else {
        setResponse(
            httpServletResponse,
            SC_BAD_REQUEST,
  ...
  private boolean initProjectWithConfiguration(
    ...
    validateInput(input);
    if (!initProject(gitRepositoryName, headName, false, input.getStoreRefLog())) {
      return false;
    }
